    ...
    for j = 0, 2 do
        for i = 1, 3 do
            if not craft_list[3 * j + i]:is_empty() then
                if item == nil then
                    item = craft_list[3 * j + i]
                elseif item:get_name() ~= craft_list[3 * j + i]:get_name() then
                    return false, ""
                end
            end
        end
...
local function craft_organize(player, formname, fields)
    ...
    if not is_creative and total_amount > 891 then
        minetest.chat_send_player(player_name, "There are too many items to organize ! Have less than 9 x 99 items.")
        return
    end
    ...
    for i = 1, 9 do
        res[i]:set_count(0)
    end -- Doing this because using empty ItemStack in list constructor crashes the game :S

    local pattern = uip.craft_patterns[tonumber(pattern_id)].pattern
    local nb_stacks = 0
    for i in pairs(pattern) do
        nb_stacks = nb_stacks + 1
    end
    local stack_size = math.floor(total_amount / nb_stacks)
    if not is_creative then
        stack_size = math.min(stack_size, ItemStack(type_name):get_stack_max())
    end -- limit stacks to get_stack_max() for non creatives
